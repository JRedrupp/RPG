...
curagua = Spell("Curagua", 50, 6000, "white")
...
while running:
            ...
            if item.type == "potion":
                ...
                if item.name == "MegaElixir":
                    for i in players:
                        i.hp = i.maxhp
                        i.mp = i.maxmp
                    print(bcolors.OKGREEN + "\n" + item.name + "fully restores all party's HP/MP" + bcolors.ENDC)
                else:
    ...
    for enemy in enemies:
        ...
        if enemy_choice == 0:
            ...
            print(enemy.name.replace(" ", "") + " attacks " + players[target].name.replace(":", "").replace(" ", "") +
                  " for", enemy_dmg)
            ...
            if not spell:
                print(bcolors.FAIL + enemy.name + "cannot afford any spells. Misses Turn")
            else:
                magic_dmg = spell.generate_dmg()
